...
def determine_available_columns(path: Path) -> List[str]:
    header = pd.read_csv(path, nrows=0)
    columns = list(header.columns)
    usecols = [col for col in DESIRED_COLUMNS if col in columns]
    if "FL_DATE" not in usecols:
...
def process_dataset(args: argparse.Namespace) -> None:
    ...
    for path in csv_files:
        usecols = determine_available_columns(path)
        parse_dates = ["FL_DATE"] if "FL_DATE" in usecols else []
        reader = pd.read_csv(
            path,
            usecols=usecols,
